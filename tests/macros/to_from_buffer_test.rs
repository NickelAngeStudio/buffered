 ...
 * @module tampon::tests
 *
 * @brief Contains unit and integration tests for to_buffer! and from_buffer! macro.
 * 
 * @details
 * Contains unit and integration tests for to_buffer! and from_buffer! macro.
 *
...
use tampon::{to_buffer, from_buffer};
...
fn to_buffer_not_enough_space(){
    ...
    to_buffer!(buffer, (to_n0):u8, (to_n1):u16, (to_n2):u32, (to_n3):u64, (to_n4):u128, (to_n5):f32, (to_n6):f64,
    (to_n7):i8, (to_n8):i16, (to_n9):i32, (to_n10):i64, (to_n11):i128);
}
...
fn to_from_buffer_2_bools(){
    ...
    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, (to_b0, to_b1):bool);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, (from_b0, from_b1):bool);
...
fn to_from_buffer_10_bools(){
    ...
    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, (to_b0,to_b1):bool, (to_b2):bool, (to_b3,to_b4,to_b5,to_b6,to_b7):bool, (to_b8,to_b9):bool);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, (from_b0,from_b1):bool, (from_b2):bool, (from_b3,from_b4,from_b5,
...
fn to_from_buffer_1_bool_slice(){
    // Create variables and get their size
    let mut var_size = 0;
    ...
    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_bs0]:bool);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_bs0]:bool);
...
fn to_from_buffer_2_bool_slices(){
    // Create variables and get their size
    let mut var_size = 0;
    ...
    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_bs0, to_bs1]:bool);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_bs0, from_bs1]:bool);
...
fn to_from_buffer_10_bool_slices(){
    // Create variables and get their size
    let mut var_size = 0;
    ...
    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_bs0,to_bs1]:bool, [to_bs2]:bool, [to_bs3,to_bs4,to_bs5,to_bs6,to_bs7]:bool, 
        [to_bs8,to_bs9]:bool);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_bs0,from_bs1]:bool, [from_bs2]:bool, [from_bs3,from_bs4,from_bs5,
...
fn to_from_buffer_1_numeric(){
    let mut var_size = 0;
    numeric_var!(var_size, to_n0:i8);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, (to_n0):i8);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, (from_n0):i8);
...
fn to_from_buffer_2_numerics(){
    let mut var_size = 0;
    numeric_var!(var_size, to_n0:f32, to_n1:i128);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, (to_n0):f32, (to_n1):i128);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, (from_n0):f32, (from_n1):i128);
...
fn to_from_buffer_all_numerics(){
    let mut var_size = 0;
    ...
    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, (to_n0):u8, (to_n1):u16, (to_n2):u32, (to_n3):u64, (to_n4):u128, (to_n5):f32, (to_n6):f64,
    (to_n7):i8, (to_n8):i16, (to_n9):i32, (to_n10):i64, (to_n11):i128);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, (from_n0):u8, (from_n1):u16, (from_n2):u32, (from_n3):u64, (from_n4):u128, (from_n5):f32, (from_n6):f64,
...
fn to_from_buffer_1_numeric_slice(){
    let mut var_size = 0;
    numeric_slice!(var_size, 0, to_ns0:i8);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_ns0]:i8);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_ns0]:i8);
...
fn to_from_buffer_2_numeric_slices(){
    let mut var_size = 0;
    numeric_slice!(var_size, 0, to_ns0:f32, to_ns1:i128, to_ns2:f32);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_ns0, to_ns2]:f32, [to_ns1]:i128);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_ns0, from_ns2]:f32, [from_ns1]:i128);
...
fn to_from_buffer_all_numeric_slices(){
    let mut var_size = 0;
    ...
    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_ns0]:u8, [to_ns1]:u16, [to_ns2]:u32, [to_ns3]:u64, [to_ns4]:u128, [to_ns5]:f32, [to_ns6]:f64,
    [to_ns7]:i8, [to_ns8]:i16, [to_ns9]:i32, [to_ns10]:i64, [to_ns11]:i128);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_ns0]:u8, [from_ns1]:u16, [from_ns2]:u32, [from_ns3]:u64, [from_ns4]:u128, [from_ns5]:f32, [from_ns6]:f64,
...
fn to_from_buffer_1_string(){
    let mut var_size = 0;
    string_var!(var_size, STRINGS, 0, to_s0);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, (to_s0):String);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, (from_s0):String);
...
fn to_from_buffer_2_strings(){
    let mut var_size = 0;
    string_var!(var_size, STRINGS, 0, to_s0, to_s1);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, (to_s0, to_s1):String);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, (from_s0, from_s1):String);
...
fn to_from_buffer_10_strings(){
    let mut var_size = 0;
    string_var!(var_size, STRINGS, 0, to_s0, to_s1, to_s2, to_s3, to_s4, to_s5, to_s6, to_s7, to_s8, to_s9);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, (to_s0, to_s1):String, (to_s2, to_s3, to_s4):String, (to_s5):String, (to_s6, to_s7, to_s8, to_s9):String);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, (from_s0, from_s1):String, (from_s2, from_s3, from_s4):String, (from_s5):String, 
...
fn to_from_buffer_1_string_slice(){
    let mut var_size = 0;
    string_slice!(var_size, STRINGS, 0, to_ss0);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_ss0]:String);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_ss0]:String);
...
fn to_from_buffer_2_string_slices(){
    let mut var_size = 0;
    string_slice!(var_size, STRINGS, 0, to_ss0, to_ss1);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_ss0, to_ss1]:String);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_ss0, from_ss1]:String);
...
fn to_from_buffer_10_string_slices(){
    let mut var_size = 0;
    string_slice!(var_size, STRINGS, 0, to_ss0, to_ss1, to_ss2, to_ss3, to_ss4, to_ss5, to_ss6, to_ss7, to_ss8, to_ss9);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_ss0, to_ss1]:String, [to_ss2, to_ss3, to_ss4]:String, [to_ss5]:String, 
        [to_ss6, to_ss7, to_ss8, to_ss9]:String);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_ss0, from_ss1]:String, [from_ss2, from_ss3, from_ss4]:String, [from_ss5]:String, 
...
fn to_from_buffer_1_tampon(){
    let mut var_size = 0;
    tampon_var!(var_size, to_t0:TamponS1);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, (to_t0):TamponS1);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, (from_t0):TamponS1);
...
fn to_from_buffer_2_tampons(){
    let mut var_size = 0;
    tampon_var!(var_size, to_t0:TamponS1, to_t1:TamponS2);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, (to_t0):TamponS1, (to_t1):TamponS2);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, (from_t0):TamponS1, (from_t1):TamponS2);
...
fn to_from_buffer_10_tampons(){
    ...
    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, (to_t0):TamponS1, (to_t1):TamponS2, (to_t2, to_t3, to_t4):TamponS1,
        (to_t5, to_t6):TamponS2, (to_t7, to_t8):TamponS1, (to_t9):TamponS2);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, (from_t0):TamponS1, (from_t1):TamponS2, (from_t2, from_t3, from_t4):TamponS1,
...
fn to_from_buffer_1_tampon_slice(){
    let mut var_size = 0;
    tampon_slice!(var_size, 0, to_ts0:TamponS1);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_ts0]:TamponS1);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_ts0]:TamponS1);
...
fn to_from_buffer_2_tampon_slices(){
    let mut var_size = 0;
    tampon_slice!(var_size, 0, to_ts0:TamponS1, to_ts1:TamponS1, to_ts2:TamponS2);

    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_ts0, to_ts1]:TamponS1, [to_ts2]:TamponS2);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_ts0, from_ts1]:TamponS1, [from_ts2]:TamponS2);
...
fn to_from_buffer_10_tampon_slices(){
    let mut var_size = 0;
    ...
    // Create buffer with to_buffer!
    let mut buffer:Vec<u8> = vec![0;var_size];
    to_buffer!(buffer, to_size, [to_ts0]:TamponS1, [to_ts1]:TamponS2, [to_ts2, to_ts3, to_ts4]:TamponS1, [to_ts5, to_ts6]:TamponS2,
        [to_ts7, to_ts8]:TamponS1, [to_ts9]:TamponS2);

    // Get back data with from_buffer!
    from_buffer!(buffer, from_size, [from_ts0]:TamponS1, [from_ts1]:TamponS2, [from_ts2, from_ts3, from_ts4]:TamponS1, [from_ts5, from_ts6]:TamponS2,
